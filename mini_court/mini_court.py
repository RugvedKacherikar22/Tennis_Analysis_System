...
from utils.bbox_utils import get_centre_of_bbox
sys.path.append('../')
import constants
from utils import (
    convert_meters_to_pixel_distance,
    convert_pixel_distance_to_meters,
    get_foot_position_box,
...
class MiniCourt():
    ...
    def set_mini_court_position(self):
        ...
        self_court_end_y = self.end_y - self.padding_court
    ...
    def draw_court(self, frame):
        for i in range (0, len(self.drawing_key_points)-1, 2):
    ...
    def convert_bounding_boxes_to_mini_court_coordinates(self, player_boxes, ball_boxes, original_court_keypoints):
        ...
        for frame_num, player_bbox in enumerate(player_boxes):
            ...
            for player_id, bbox in player_bbox.items():
                foot_position = get_foot_position_box(bbox)

                # ✅ Different keypoints per player
                if player_id == 2:
                    keypoints_to_check = [0, 1, 4, 5]
                else:
                    keypoints_to_check = [2 ,5, 10, 13]

                # print(f"Player {player_id} using keypoints: {keypoints_to_check}")
                # print(f"Player {player_id} foot position: {foot_position}")

                closest_key_point_index = get_closest_keypoint_index(foot_position, original_court_keypoints, keypoints_to_check)
                ...
                if closest_player_id_to_ball == player_id:
                    closest_key_point_index = get_closest_keypoint_index(ball_position, original_court_keypoints, keypoints_to_check)
                    closest_key_point = (original_court_keypoints[closest_key_point_index*2],
                                        original_court_keypoints[closest_key_point_index*2+1])
            ...
            output_player_boxes.append(output_player_bboxes_dict)  # ✅ outside player loop

        return output_player_boxes, output_ball_boxes  # ✅ both returned
